...
//HANDLE ERRORS RECEIVED FROM ARDUINO TO UBUNTU
...
//{
//	int32_t data[16];
//	memcpy(data, &buffer[1], sizeof(data)); // memcpy(Dest,source, size)
//
//	printf("\n ------- ALEX STATUS REPORT ------- \n\n");
...
//Arduino will send this message to UBUNTU 
...
//sendData from ubuntu to PI -> Arduino
...
// read data from arduino in Ubuntu 
...
//						getParams(params);
//						buffer[1] = ch;
//						memcpy(&buffer[2], params, sizeof(params)); // memcpy(Dest,source, size)
//						sendData(conn, buffer, sizeof(buffer));
//						break;
//			case 's': //stop
//			case 'S':
...
// put in ip address of PI and port number : 5000 
