...
//send packet from PI to Arduino 
...
//receive error from Arduino to PI 
...
void sendNetworkData(const char *data, int len) // from PI to laptop 
{
...
//handle command of laptop to PI
void handleCommand(void *conn, const char *buffer)
{
	...
	//printf("COMMAND RECEIVED: %c %d %d\n", cmd, cmdParam[0], cmdParam[1]); // param[0] : distance  param[1]: power
...
//			uartSendPacket(&commandPacket); // communication through UART from PI to Arduino 
//			break;
//
//		case 'b':
//		case 'B':
...
int main()
{
	...
	printf("Done. Waiting 2 seconds for Arduino to reboot\n");
	sleep(2);
